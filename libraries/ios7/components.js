...
prx.types.ios7_button_bg = {
	...
	,onDisplay: function(item,containerid,symbol) {
        ...
        var _props = (typeof(prx.richtext)=='undefined') ? prx.componentsHelper.getProp(item.textProperties,'props-text') : '';
        var text = '<div class="ios7-button-text liveUpdate-textColor-color changeProperty-textColor changeProperty-textFont changeProperty-textFontStyle changeProperty-textSize"><span data-editableproperty="text">'+prx.componentsHelper.getProp(item.text,'text-textarea')+'</span></div>';
        var icon = '';
        ...
        cR += '<style>';
        cR += prx.items.getComponentBaseStyle(item, containerid, symbol);
        cR += '#'+_id+' .ios7-button-inner { '+prx.css.flexJustifyContent(prx.componentsHelper.getProp(item.textAlign,'align'))+' text-align: '+prx.componentsHelper.getProp(item.textAlign,'align')+'; '+prx.utils.getBgCss(item.backgroundColor)+'; border: '+prx.componentsHelper.getProp(item.borderWidth,'num-border-width')+'px solid '+prx.componentsHelper.getProp(item.borderColor,'color-border')+'; border-radius: '+prx.componentsHelper.getProp(item.borderRadius,'border-radius')+'; }'
        if (typeof(item.iconpos) != 'undefined' && item.iconpos != 'notext') {
            cR += '#'+_id+' .ios7-button-text { '+prx.componentsHelper.getProp(item.textFont + '|' + item.textFontStyle,'font-family')+prx.componentsHelper.getProp(item.textFontStyle,'font-style')+' color: '+prx.componentsHelper.getProp(item.textColor,'color-text')+'; font-size: '+prx.componentsHelper.getProp(item.textSize,'num-text-size')+'px; '+_props+' }';
        }
        cR += '</style>';
        cR += prx.items.getComponentPrependDivs(item, containerid, symbol);

		cR += '<div class="ios7-button-inner ios7-button-icon-'+prx.componentsHelper.getProp(item.iconpos,'icon-position')+' liveUpdate-backgroundColor-background-color liveUpdate-borderColor-border-color changeProperty-backgroundColor changeProperty-borderColor changeProperty-borderWidth changeProperty-borderRadius">';
                    ...
                    ,value: function(item,name) {
                      ...
                      ,onChange: function(item){
                          ...
                          if(item.iconpos == 'notext') {
                              $('#property-textSize, #property-textFont, #property-textFontStyle, #property-textColor, #property-textProperties, #property-textAlign').hide();
                          } else {
                              $('#property-textSize, #property-textFont, #property-textFontStyle, #property-textColor, #property-textProperties, #property-textAlign').show();
                          }
                          return false;
                      }
                      ,changeProperty: {
...
prx.componentsHelper.removeProperties(prx.types.ios7_button_icon_square.propertyGroups, ['textFont','textSize', 'textFontStyle', 'textColor','textProperties','textAlign','iconpos'])
...
prx.types.ios7_segmentedcontrol = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        cR += '<div class="ios7-segmentedcontrol-inner liveUpdate-backgroundColor-background-color liveUpdate-textColor liveUpdate-borderColor-border-color changeProperty-backgroundColor changeProperty-borderColor changeProperty-borderRadius changeProperty-borderWidth changeProperty-textColor changeProperty-textSize changeProperty-textFont">';

        $.each(item.buttons, function(i,elm) {
            cR += '<div id="'+_id+'-buttons-'+i+'" class="ios7-segmentedcontrol-button dynamic-property liveUpdate-borderColor-border-color'+((i == 0 || i == item.buttons.length-1)? ' changeProperty-borderRadius' : '')+'" data-dynamic-property-index="'+i+'">';
            ...
            cR += '<label for="'+_id+'-radio-'+i+'" '+((prx.componentsHelper.getProp(item.selected,'num-other') == i) ? 'class="liveUpdate-activeBackgroundColor-background-color liveUpdate-activeTextColor-color changeProperty-activeBackgroundColor changeProperty-activeTextColor"' : '')+' >'
...
prx.types.ios7_slider = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
		...
		cR += '<div class="slider-bar liveUpdate-barColor-background-color">';
		cR += '<div class="slider-bar-filled liveUpdate-fillBarColor-background-color"></div>';
		cR += '</div>';
		cR += '<span class="slider-button liveUpdate-sliderColor-background-color"></span>';
		cR += prx.items.getComponentAppendDivs(item, containerid, symbol);
		cR += '</div>';
		return cR;
	}
			...
			//if (typeof(prx.scrollable._scrollables[_id + '-inner'])=='undefined') {
				try {
                    ...
                    if(item.selectedValue <= _options.length && item.selectedValue!=-1) {
                        prx.scrollable._scrollables[_id + '-inner'].scrollTo(0,-item.selectedValue * (32*prx.componentsHelper.getScale(item.lib)), 150*prx.componentsHelper.getScale(item.lib));
                        $('#'+_id).trigger('pickerchange');
            ...
            properties: [
                ...
                [
                    ...
                    ,{
                        ...
                        pffSettings: prx.commonproperties.pffSettingsColor,
                        value: function(item,name){
                            return item.activeTextColorios7_listcomplex
                        }
                        ,values: prx.comps.ios7Colors
                        ,liveUpdate: 'color'
                        ,changeProperty: {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(prx.componentsHelper.getProp(item.detachlast,'boolean')) {
            ...
            if(i>0) {
                ...
                cR += '<div id="'+_id+'-buttons-'+i+'" class="ios7-actionsheet-button dynamic-property liveUpdate-borderColor-border-color liveUpdate-textColor-'+i+' changeProperty-borderColor" data-dynamic-property-index="'+i+'" style="'+_props+' color: '+prx.componentsHelper.getProp(item.buttons[i].textColor,'color-text') + ';font-size:' + prx.componentsHelper.getProp(item.buttons[i].textSize,'text-size') + 'px;">';
...
prx.components.ios7_listbasic = {
    ...
    ,textProperties: []
      ...
      ,subFont: 'HelveticaNeue, Helvetica, Verdana, Arial, sans-serif'
      ,subColor: '999999'
      ,subProperties: []
    ,subFontStyle: '300'
    ,backgroundColor: 'FFFFFF'
...
prx.components.ios7_listwithvalue = {
    ...
    ,valueFont: 'HelveticaNeue, Helvetica, Verdana, Arial, sans-serif'
    ,valueFontStyle: '300'
    ,valueColor: '999999'
    ,valueProperties: []
    ,backgroundColor: 'FFFFFF'
...
prx.components.ios7_listwithbadge = {
    ...
    ,badgeFont: 'HelveticaNeue, Helvetica, Verdana, Arial, sans-serif'
    ,badgeFontStyle: '300'
    ,badgeColor: 'ffffff'
    ,badgeProperties: []
...
prx.components.ios7_alert = {
    ...
    ,textSize: 17*prx.componentsHelper.getScale(_library)
    ,textColor: '#333333'
    ,title: '<span style="font-weight: 700">Alert title</span>'
    ,text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit'
    ,buttons: [{
        actions: [],
        text: 'Cancel',
